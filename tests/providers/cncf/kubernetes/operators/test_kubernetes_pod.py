...
class TestKubernetesPodOperator:
    ...
    def test_templates(self, create_task_instance_of_operator):
        ...
        assert 'my_memory_limit' == rendered.k8s_resources.limits['memory']
        assert 'my_cpu_limit' == rendered.k8s_resources.limits['cpu']
        assert 'my_requested_memory' == rendered.k8s_resources.requests['memory']
        assert 'my_requested_cpu' == rendered.k8s_resources.requests['cpu']
    ...
    def test_templates_backward_compatibility(self, create_task_instance_of_operator):
        ...
        ti = create_task_instance_of_operator(
            ...
            namespace='{{ dag.dag_id }}',
            container_resources={
                'request_memory': 'my_requested_memory',
                'request_cpu': 'my_requested_cpu',
                'limit_cpu': 'my_cpu_limit',
                'limit_memory': 'my_memory_limit',
            },
            pod_template_file='{{ dag.dag_id }}',
            config_file='{{ dag.dag_id }}',
            labels='{{ dag.dag_id }}',
